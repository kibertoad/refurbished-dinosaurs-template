...
2. Each slice in the plan names the spec areas or entries it needs and the
   ...
   format rows can reach `validated` but rule and screen rows stop at
   `implemented`, and a row that needs a live session nobody has accepted
   ...
   are statements a script or reviewer can check. Put questions only the owner can
...
1. For every spec entry below `established` whose Open questions has something
   ...
   it names. A question a static reading can settle goes under `Static`; a run
   goes under `Agent run` or `Live session` as `docs/RUNTIME.md` says. Items
   duplicating one another are merged.
